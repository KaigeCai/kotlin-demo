...
class Game {
    ...
    // 加载游戏
    fun loadGame() {
        if (File("SaveGame.txt").exists()) {
            val lines = File("SaveGame.txt").readLines()
            player.name = lines[0]
            player.health = lines[1].toInt()
            player.gold = lines[2].toInt()
            player.inventory = lines[3].split(",").toMutableList().filter { it.isNotBlank() }.toMutableList()
            currentLocation = locations[lines[4]] ?: locations["村庄"]!!
            println("游戏已加载！")
        } else {
            println("没有找到保存的游戏。")
        }
